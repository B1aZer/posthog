...
SQL results can inform how you construct a later typed query. Typed query tools cannot accept SQL results as input. Reassess the method when the task changes, regardless of the previous tool call. A chart or table alone does not determine the method: both typed queries and SQL can support saved visualizations.
