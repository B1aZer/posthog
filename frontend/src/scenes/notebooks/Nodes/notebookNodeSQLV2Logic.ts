...
export const notebookNodeSQLV2Logic = kea<notebookNodeSQLV2LogicType>([
    ...
    listeners(({ props, actions, cache, values }) => {
        ...
        return {
            ...
            // A run the backend started for this cell as part of a whole-notebook run. The cell
            // did not dispatch it, so it pins the run id into the document and picks up the
            // existing poll path, which writes the result and reports the outcome as usual.
            // A cell that is not mounted adopts nothing; its runId reaches the document on the
            // next poll that still sees the run, and afterMount recovers it from there.
            adoptChainRun: ({ nodeId, runId }) => {
                if (nodeId !== props.nodeId || cache.activeRunId === runId) {
                    return
                }
