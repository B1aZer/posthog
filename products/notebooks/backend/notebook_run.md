...
- That same read stops a run whose notebook was deleted. A deleted notebook is hidden from the run's status and interrupt endpoints, so nothing else could end the run, and its cells would keep running on paid compute until the hour was up.
...
- A run whose record is older than the workflow's Temporal execution timeout is closed as `failed` when the next run starts. A terminated execution writes no outcome, and without that reclaim the partial unique constraint would refuse every later run of the notebook.
- `scratchpad` returns 400. The browser holds it, so the server has no cells to plan and no row to save variables against.
