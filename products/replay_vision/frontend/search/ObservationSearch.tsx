...
import { crossScannerSearchUrl } from './observationQueries'
    ...
    return (
        ...
        >
            <input
                ...
                data-attr={`${dataAttrPrefix}-query`}
            />
            {query && (
                <LemonButton
                    size="xsmall"
                    ...
                    onClick={onClear}
                    tooltip="Clear"
                    data-attr={`${dataAttrPrefix}-clear`}
                />
            )}
            <LemonButton
                type="primary"
                ...
                tooltip="Search"
                data-attr={`${dataAttrPrefix}-submit`}
            />
        </label>
    )
}
...
export function ObservationSearch({
    ...
    return (
        <div
            ...
            data-attr={attr}
        >
            {/* The prompt keeps its half width in every state, so the box does not jump as the card changes. */}
            <div className="grid gap-4 @3xl:grid-cols-[minmax(0,2fr)_minmax(0,1fr)_minmax(0,1fr)]">
                <div className="flex flex-col gap-2">
                    <SearchPrompt
                        inputId={`${attr}-${scannerId ?? 'all'}`}
                        query={query}
                        searching={searching}
                ...
                {idle && (
                    <>
                        ...
                        {suggestedQueriesLoading ? (
                            // Hold the strip rather than flash the fixed examples and swap them out a moment later.
            ...
            {results?.length === 0 &&
                (crossScanner ? (
                    ...
                    <div className="rounded bg-surface-secondary px-3 py-2 text-sm text-secondary">
                        ...
                        <Link
                            to={crossScannerSearchUrl(searchedQuery ?? '')}
                            data-attr="vision-observations-search-all"
                        >
                            Search all scanners
                        </Link>
                    </div>
